init python:
    ...
    class Prostitution_Client(store.object):
        def __init__(self, level = None, prefered_act='random', bonus_act='random'):
            self.level = level or random.randint(g_base.client_min_level, g_base.client_max_level)
            self.expected_money = g_base.client_money_mod*self.level
    ...
    class Prostitution_Act(store.object):
        def __init__(self, girl: Personage, client: Prostitution_Client, girl_id: int, client_id: int):
            ...
            self._roll_base = g_base.prostitution_difficulty_modifier + girl.stat[client.prefered_act].act_level() - client.level
    ...
    class Prostitution_Night(store.object):
        ...
        def generate_acts(self):
            ...
            for girl_id, girl in enumerate(g_base.girls.list):
